...
- [ ] UI 调整（2026-09-18 用户提出，三点方案已确认）：① 顶部加「工作台 / 模板制作指南」tab 切换——指南页含占位符写法说明（D3 主路径 + M3b 词表 + D6 限制）+ 可下载示例模板 docx（显式中文字体，P17）+ 可复制示例段落；② 字符块库抽屉化——左缘按钮控制展开/收起，默认展开；③ 模板选择下拉从顶栏挪到预览工具条（替换原静态文件名位置、「已绑定」图例左边），注意 idle 态（未选模板）也要能选
...
- [ ] 用户反馈（2026-09-18 验收）：块库无删除功能——已含于 M2 计划（删除二次确认+软删除 D11），M2 会话优先实现
